...
export function TaxSyncPanel() {
  ...
  const handlePreview = async () => {
    ...
    try {
      const res = await fetch("/api/tax/sync/preview", {
        method: "POST",
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch("/api/tax/sync/confirm", {
        method: "POST",
        signal: controller.signal,
      });
    ...
    <div className="flex flex-col gap-4">
      ...
      {preview && (
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          <h3 className="mb-3 text-sm font-semibold text-slate-800">Prévia da sincronização</h3>
          <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6">
            ...
            <div className="rounded bg-white p-2 shadow-sm">
              <div className="text-xs text-slate-500">Válidas</div>
            ...
            <div className="rounded bg-white p-2 shadow-sm">
              <div className="text-xs text-slate-500">A ignorar</div>
              <div className="text-lg font-semibold text-slate-600">{preview.wouldSkip}</div>
            </div>
          </div>

          {preview.errors.length > 0 && (
            <div className="mb-4">
              <h4 className="mb-2 text-xs font-medium uppercase text-slate-500">Erros (até 10)</h4>
                  ...
                  <tbody>
                    {preview.errors.map((e, i) => (
                      <tr key={i} className="border-t border-slate-100">
                        ...
                        <td className="px-3 py-1 text-slate-600">{e.field ?? "—"}</td>
          ...
          {preview.sample.length > 0 && (
            <div className="mb-4">
              <h4 className="mb-2 text-xs font-medium uppercase text-slate-500">Amostra (3)</h4>
              <ul className="space-y-1 rounded border border-slate-200 bg-white p-3 text-sm">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-slate-700">Entendo que isso vai atualizar a base</span>
            </label>
            <button
              type="button"
              onClick={handleConfirm}
              disabled={!confirmed || loadingConfirm}
              className="w-fit rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
