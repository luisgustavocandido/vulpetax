...
export function SearchableCountrySelect({
  ...
  const [selected, setSelected] = useState(() => {
    const found = UNIQUE_COUNTRY_CODES.find((c) => c.code === defaultValue);
    return found ?? UNIQUE_COUNTRY_CODES[0];
  });
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className="flex w-full items-center justify-between rounded border border-gray-300 bg-white px-2 py-1.5 text-left text-sm hover:border-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
      >
        <span>
              ...
              filtered.map((c) => (
                <button
                  ...
                  className={`block w-full px-3 py-2 text-left text-sm hover:bg-blue-50 ${
                    selected.code === c.code ? "bg-blue-50 font-medium text-blue-700" : ""
                  }`}
                >
                  {c.name} {c.code}
                </button>
