...
type BusinessTypeComboboxProps = {
  ...
  options: readonly string[]; // BUSINESS_TYPES
...
export function BusinessTypeCombobox({
  ...
  // Filtrar opções (exceto "Outro" que sempre aparece no final)
  const filteredOptions = useMemo(() => {
    const otherOptions = options.filter((opt) => opt !== "Outro");
                ...
                return (
                  <button
                    ...
                    className={`block w-full px-3 py-2 text-left text-sm transition-colors ${
                      ...
                      isOutro ? "border-t border-gray-200 mt-1 pt-2" : ""
                    } hover:bg-blue-50`}
                    role="option"
