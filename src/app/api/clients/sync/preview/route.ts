...
import {
  fetchPosVendaRows,
  buildPosVendaPreview,
} from "@/lib/sync/runPosVendaSync";
import {
  clientsSyncPreviewRateLimitCheck,
  clientsSyncPreviewRateLimitConsume,
...
/** POST /api/clients/sync/preview — pré-visualização sem gravar. Protegido por passcode (middleware). */
export async function POST(request: NextRequest) {
  const ip = getIp(request);

  if (!clientsSyncPreviewRateLimitCheck(ip)) {
  ...
  clientsSyncPreviewRateLimitConsume(ip);
