...
import { listAnnualReports, getAnnualReportSummary } from "@/lib/billing/annualReportRepo";
import { ensureAnnualReportObligations, getAnnualReportDebugInfo } from "@/lib/billing/annualReportEngine";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await ensureAnnualReportObligations({ windowMonths });
    ...
    const { data, total } = await listAnnualReports(filters);

    // Summary com filtros aplicados (exceto paginação e status)
    const summary = await getAnnualReportSummary({
    ...
    const response: {
      data: typeof data;
      meta: { page: number; limit: number; total: number; totals: { pending: number; overdue: number; done: number } };
      debug?: unknown;
    } = {
      data,
      meta: {
    ...
    if (debug) {
      const debugInfo = await getAnnualReportDebugInfo();
      response.debug = {
        ...debugInfo,
        engineResult: result,
      };
    ...
    console.error("[GET /api/billing/annual-reports]", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Erro ao listar obrigações de Annual Report" },
      { status: 500 }
    );
  }
}
